...
export default function CallCenterLogin() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const { data } = await api.post("/agent/login", { email, password });
    ...
    <div className={styles.container}>
      <div className={styles.leftPanel}>
        <div className={styles.brandContent}>
          <div className={styles.logoBlock}>
            <div className={styles.camelIcon}>
              <svg viewBox="0 0 60 60" width="48" height="48">
                <circle cx="30" cy="30" r="28" fill="url(#sunGrad2)" />
                <defs>
                  <linearGradient id="sunGrad2" x1="0" y1="0" x2="0" y2="1">
          ...
          <p className={styles.brandTagline}>
            Generate pre-authorization codes, verify members,
            and manage reimbursement claims.
          </p>
        </div>
      </div>
      ...
      <div className={styles.rightPanel}>
        <form className={styles.form} onSubmit={handleSubmit}>
          ...
          <p className={styles.formSubtitle}>Access the call center dashboard</p>
