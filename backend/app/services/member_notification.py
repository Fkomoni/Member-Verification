...
def _build_chronic_email_html(
    ...
    """Build HTML email for chronic medication requests."""
    med_rows = ""
    for i, m in enumerate(medications, 1):
        med_rows += f"""<tr>
            <td style="padding:10px;border:1px solid #e0e0e0">{i}</td>
...
<div style="font-family:Arial,sans-serif;max-width:650px;margin:0 auto;background:#fff;border:1px solid #e0e0e0;border-radius:8px;overflow:hidden">
    <div style="background:#C61531;padding:20px;text-align:center">
        <h1 style="color:#fff;margin:0;font-size:22px">LEADWAY Health</h1>
        <p style="color:#FFE0E0;margin:5px 0 0;font-size:13px">Medication Request Notification</p>
    </div>
    <div style="padding:25px">
        <p style="font-size:15px">Dear <strong>{member_name}</strong>,</p>
        <p style="color:#555;line-height:1.6">We hope this message finds you well.</p>
        <p style="color:#555;line-height:1.6"><strong>{facility_name}</strong> has submitted a medication request on your behalf through the Leadway Health portal.</p>

        <div style="background:#F8F9FA;border-radius:8px;padding:15px;margin:20px 0;border-left:4px solid #C61531">
            <table style="width:100%;font-size:14px">
                <tr><td style="padding:4px 0;color:#777;width:140px">Reference:</td><td style="font-weight:bold">{reference}</td></tr>
                <tr><td style="padding:4px 0;color:#777">Diagnosis:</td><td>{diagnosis}</td></tr>
                <tr><td style="padding:4px 0;color:#777">Member Phone:</td><td>{member_phone}</td></tr>
                <tr><td style="padding:4px 0;color:#777">Delivery Address:</td><td>{delivery_address}</td></tr>
            </table>
        </div>
        ...
        <p style="color:#555;line-height:1.6;margin-top:25px">We have received your prescription and our pharmacy team is currently reviewing your order. We will be in touch with you shortly to confirm availability and arrange delivery or pickup.</p>

        <p style="color:#555;line-height:1.6">If you have any questions in the meantime, please don't hesitate to contact us.</p>

        <p style="color:#555;line-height:1.6;margin-top:25px">Warm regards,<br><strong>Leadway Health Pharmacy Team</strong><br><em>For health, wealth &amp; more...</em></p>
...
async def send_chronic_member_email(
    request_id: str,
    db: Session,
):
    """Send chronic request notification email via Prognosis SendEmailAlert API."""
    req = db.query(MedicationRequest).filter(MedicationRequest.request_id == request_id).first()
    if not req or not req.member_email:
        logger.info("No email for member — skipping chronic notification")
        return
    ...
    html = _build_chronic_email_html(
        member_name=req.enrollee_name,
        member_phone=req.member_phone or "",
        delivery_address=req.delivery_address or "",
        facility_name=req.facility_name,
    ...
    token = await _get_prognosis_token()
    if not token:
        logger.error("Cannot send chronic email — no Prognosis token")
        return
    ...
    email_payload = {
        ...
        "ServiceId": 0,
        "Reference": req.reference_number,
        "TransactionType": "ChronicFulfilment",
    }
    ...
    try:
        ...
        logger.info("Chronic email API response: %d %s", resp.status_code, resp.text[:200])

        db.add(MedicationAuditLog(
            event_type="chronic_member_email_sent",
            request_id=request_id,
            detail=f"Chronic email sent to {req.member_email}. Status: {resp.status_code}",
        ))
        db.flush()

    except Exception as e:
        logger.error("Chronic email send failed: %s", e)
